...
export default function TestimonialSection() {
  ...
  // Update itemsPerScreen based on window size
  useEffect(() => {
    const updateScreenSize = () => {
      setItemsPerScreen(window.innerWidth < 768 ? 1 : 3);
    };
    updateScreenSize();
  ...
  const displayedTestimonials = [
    ...testimonials.slice(currentIndex, currentIndex + itemsPerScreen),
    ...testimonials.slice(0, Math.max(0, currentIndex + itemsPerScreen - testimonials.length)),
  ];
      ...
      <div className="relative max-w-6xl mx-auto px-4 pt-4 overflow-hidden">
        <AnimatePresence mode="popLayout">
          <motion.div
            ...
            className={`grid gap-6 ${itemsPerScreen === 1 ? "grid-cols-1" : "grid-cols-3"}`}
          >
            {displayedTestimonials.map((testimonial) => (
              <motion.div
                key={testimonial.id}
                className="p-6 bg-white shadow-md shadow-blue-900 rounded-2xl relative h-60"
              >
